...
<h4 class="card-title">Sistema de Servicio Comunitario | {{ $sedeprincipal  }}   
          ...
          <div class="card-body">
            <div class="row">
              <div class="col-12 text-right">
                ...
                <a href="{{ route('Prestadores.create') }}" class="btn btn-sm btn-facebook">Añadir Estudiantes</a>
    ...
    <tr>
         <td>{{$el->ci}}</td>
         <td>{{$el->firstname}} {{$el->middlename}}</td>
         <td>{{$el->primary_lastname}} {{$el->second_lastname}}</td>
         <td>{{$el->grupo_id}}</td>
          <td>{{$el->nombre_proyecto}}</td>
          <td>{{$el->nombre}}</td>
          <td class="td-actions text-right">
    <a href="{{ route('Prestadores.edit', $el->id) }}" class="btn btn-warning"><i class="material-icons">edit</i></a>

            <form action="{{ route('Prestadores.destroy', $el->id) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('Seguro?')">
                                @csrf
                                @method('DELETE')
